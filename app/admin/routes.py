...
def list_organizations(request: Request):
    ...
    res = supabase.table("users").select("username, organization, is_admin").execute()
...
def delete_organization(organization: str, request: Request):
    ...
    supabase.table("users").delete().eq("organization", organization).execute()

    logger.warning(f"Admin deleted organization: {organization}")

    return {"message": f"Organization '{organization}' deleted permanently"}
