...
def export_orders_pdf(
    ...
    query = supabase.table("orders") \
        .select("order_id,date,receiver_name,product,quantity,total_amount_with_gst,advance_payment,pending_amount,status") \
        .eq("org", org)
    if status and status.lower() != "all":
        query = query.eq("status", status)
    if start_date:
    ...
    cols = ["#", "Date", "Customer", "Product", "Qty", "Total (Rs.)", "Advance (Rs.)", "Pending (Rs.)", "Status"]
    rows = []
    ...
    col_w = [10*mm, 22*mm, 55*mm, 50*mm, 13*mm, 30*mm, 30*mm, 30*mm, 23*mm]
    pdf_bytes = build_pdf("Orders Report", cols, rows, org_info, col_widths=col_w, landscape_mode=True)
    return StreamingResponse(
...
def export_revenue_summary(
    ...
    org = require_login(request)
    orders = supabase.table("orders").select("date,total_amount_with_gst").eq("org", org).execute().data
    if not orders:
    ...
    monthly = df.groupby("month")["total_amount_with_gst"].sum().reset_index(name="monthly_revenue")
    yearly  = df.groupby("year")["total_amount_with_gst"].sum().reset_index(name="yearly_revenue")
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
...
def export_revenue_summary_pdf(
    ...
    org_info = get_org_info(org)
    df = pd.DataFrame(orders) if orders else pd.DataFrame(columns=["date", "total_amount_with_gst"])
    if not df.empty:
        df["date"] = pd.to_datetime(df["date"])
        df["year"] = df["date"].dt.year
        df["month"] = df["date"].dt.strftime("%Y-%m")
    ...
    monthly = df.groupby("month")["total_amount_with_gst"].sum().reset_index(name="revenue") if not df.empty else pd.DataFrame()
...
def export_deliveries_pdf(
    ...
    query = supabase.table("deliveries") \
        .select("order_id,delivery_id,delivery_date,delivery_quantity,total_amount_received") \
        .eq("org", org)
    if start_date:
        query = query.gte("delivery_date", start_date)
    if end_date:
...
def export_deliveries_for_order(order_id: int, request: Request):
    ...
    deliveries = supabase.table("deliveries").select("*") \
        .eq("order_id", order_id).eq("org", org).execute().data
    if not deliveries:
    ...
    return StreamingResponse(
        io.BytesIO(excel_bytes),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f"attachment; filename=order_{order_id}_deliveries.xlsx"}
    )
...
def export_customer_statement(request: Request, customer: str):
    ...
    df = pd.DataFrame(orders)
    display_cols = ["order_id","date","product","quantity","total_amount_with_gst",
                    "advance_payment","pending_amount","status","expected_delivery_date"]
    df = df[[c for c in display_cols if c in df.columns]]
    excel_bytes = df_to_excel_bytes(df, "Customer Statement")
    return StreamingResponse(
        io.BytesIO(excel_bytes),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f"attachment; filename=statement_{customer}.xlsx"}
    )
...
def export_customer_statement_pdf(request: Request, customer: str):
    ...
    cols = ["Order#", "Date", "Product", "Qty", "Value (Rs.)", "Paid (Rs.)", "Pending (Rs.)", "Status"]
    rows = []
    for o in orders:
    ...
    # Summary row
    rows.append(["", "", "TOTAL", "", f"Rs.{total_value:,.0f}", f"Rs.{total_paid:,.0f}", f"Rs.{total_pending:,.0f}", ""])
    ...
    pdf_bytes = build_pdf(
        ...
        cols, rows, org_info, col_widths=col_w, landscape_mode=True
    )
    return StreamingResponse(
        io.BytesIO(pdf_bytes),
        media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=statement_{customer}.pdf"}
    )
...
def export_gst_report(request: Request, start_date: Optional[str] = None, end_date: Optional[str] = None):
    org = require_login(request)
    query = supabase.table("orders") \
    ...
    df = df.rename(columns={
        ...
        "price": "Unit Price (Rs.)", "gst": "GST %",
        "basic_price": "Basic Price (Rs.)",
        "gst_amount": "GST Amount (Rs.)", "total_amount_with_gst": "Total (Rs.)"
    })
    slabs = df.groupby("GST %").agg(
        Orders=("Date", "count"),
    ...
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        export_df = df[["Date","Product","Qty","Unit Price (Rs.)","Basic Price (Rs.)","GST %","GST Amount (Rs.)","Total (Rs.)"]]
        ...
        for col_idx, val in enumerate(totals, start=1):
            ...
            cell.fill = __import__("openpyxl").styles.PatternFill("solid", fgColor="E0E7FF")
...
def export_gst_report_pdf(request: Request, start_date: Optional[str] = None, end_date: Optional[str] = None):
    org = require_login(request)
    query = supabase.table("orders") \
        .select("date,product,quantity,price,gst,total_amount_with_gst") \
        .eq("org", org)
    ...
    org_info = get_org_info(org)
    cols = ["Date", "Product", "Qty", "Unit Price", "Basic Amt", "GST%", "GST Amt", "Total"]
    rows = []
    for o in orders:
    ...
    total_basic    = sum(round(o.get("quantity", 0) * o.get("price", 0), 2) for o in orders)
    total_gst_amt  = sum(round(o.get("quantity", 0) * o.get("price", 0) * o.get("gst", 0) / 100, 2) for o in orders)
    ...
    pdf_bytes = build_pdf("GST Tax Report", cols, rows, org_info, col_widths=col_w, landscape_mode=True)
    return StreamingResponse(
        io.BytesIO(pdf_bytes),
        media_type="application/pdf",
...
def export_pending_orders_pdf(request: Request):
    org = require_login(request)
    orders = supabase.table("orders") \
        .select("order_id,date,receiver_name,product,quantity,delivered_quantity,pending_amount,expected_delivery_date") \
        .eq("org", org) \
        .eq("status", "Pending") \
        .execute().data or []
    ...
    cols = ["#", "Order ID", "Customer", "Product", "Rem. Units", "Pending (Rs.)", "Due Date", "Days Left"]
    rows = []
    ...
    return StreamingResponse(
        io.BytesIO(pdf_bytes),
        media_type="application/pdf",
        headers={"Content-Disposition": "attachment; filename=pending_orders_urgent.pdf"}
    )
...
def get_customer_list(request: Request):
    org = require_login(request)
    orders = supabase.table("orders").select("receiver_name").eq("org", org).execute().data or []
